...
def get_lines(file): # get data in form of lines
    def extract_lines(input_file):# extract lines from file
        ...
        chars = str.maketrans({'{': '', '}': '', ',': '', '\"':'','\"':'', '\'':'', '-':' '})
        clean = [item.translate(chars) for item in joined_lines] # gets rid of punctuation like {} brackets and double quotes ""
        return clean
...
def write_file(file, list_data, format): # writes data to another file
    with open(f'{file}', 'w') as outfile:
        if f'{format}' is 'tuples':
            for tup in list_data:
                for item in tup:
                    outfile.write(str(item[0]) + '\n')   
            print("Successfully created file :)")
        if f'{format}' is 'strings':
...
def get_gen_summary(list_of_HL):
    ...
    write_file('headlines_WC.txt', hl_WC, format='strings')
    hl_CL = findCL(list_of_HL, paired=False) # list of character length averages of every headline
    write_file('headlines_CL.txt', hl_CL, format='strings')
...
def get_cat_summary(list_of_entries, category):
    ...
    write_file(f'{category}_headlines.txt', data, format='strings') # writes new file
    ...
    write_file(f'{category}_CL.txt', data_WC, format='strings')
    if len(data_CL) == 0:
        print('error! something went wrong:(')
...
### LISTS OF THINGS:
entries = get_lines('/Users/rena/Desktop/COURSES/LING 250/FINAL PROJECT/headlines.txt')

#### SETS OF VARIABLES ####
links = find_type(entries, '$A')

headlines = find_type(entries, '$B')
write_file('headlines.txt', headlines, format='strings')
HLwords = combine(headlines) # all words in all headlines

categories = find_type(entries, '$C')
category_list = nltk.FreqDist(categories)
print(category_list.most_common(5))
category_list.tabulate()
